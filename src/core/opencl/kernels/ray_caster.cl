...
inline bool intersect(Hit* hit, Ray ray, __global Metadata* meta_prims, __global float* prims, int nb_prims) {
	float temp;
	hit->t = 1e5f;
	hit->selfisect = false;
...
__kernel void display_samples(image2d_t env, __global const Distribution1D* pConditionalV,
		__global Distribution1D* pMarginal, __global const float* cdfConditionalV,
		__global const float* cdfMarginal, __global const float* fun2D,
		__global const float* fun1D, __global int2* derp) {

	int id = get_global_id(0);

	threefry4x32_key_t k = {{id, 0xdecafbad, 0xfacebead, 0x12345678}};
	threefry4x32_ctr_t c = {{id, 0xf00dcafe, 0xdeadbeef, 0xbeeff00d}};

	threefry4x32_ctr_t r;

	float mapPdf;
	r = threefry4x32(c , k);
	float u =  u01_open_open_32_24(r.v[0]);
	float v =  u01_open_open_32_24(r.v[1]);

	float2 sample = sampleContinuous2D(u, v, pConditionalV, *pMarginal, cdfConditionalV,
			cdfMarginal, fun2D, fun1D, &mapPdf);
	sample *= (float2)(512, 1024);
//	printf((__constant char *)"%v2f\n", sample);
	derp[id] = convert_int2(sample);
}
